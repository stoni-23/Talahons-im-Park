...
*/
var DEFAULT_APP_NAME = "Talahons im Park";
...
function appNameFromHost(hostHeader) {
	...
	return slug.split("-").filter(Boolean).map((part) => part.charAt(0).toUpperCase() + part.slice(1)).join(" ") || "Talahons im Park";
}
...
function resolveOgTitle(site = {}, appName = DEFAULT_APP_NAME, host = "", documentTitle = "") {
	...
	if (fromHost && fromHost !== "Talahons im Park") return fromHost;
	return String(appName ?? "").trim() || "Talahons im Park";
}
...
function normalizeHeadContext(ctx = {}) {
	...
	return {
		appName: resolveOgTitle(site, ctx.appName ?? "Talahons im Park", ctx.host ?? ""),
...
function injectGrokPwaHead(html, ctx = {}) {
	...
	const appName = resolveOgTitle(site, ctx.appName ?? "Talahons im Park", host, documentTitle);
	let next = stripShareMetaTags(html);
	const missing = grokPwaHeadTags(appName).filter(([key]) => {
		if (key === "manifest") return !next.includes("href=\"/__grok/manifest.webmanifest\"");
		if (key === "apple-touch-icon") return !next.includes("href=\"/logo.png\"");
...
var _lazy_IPAi_L = defineLazyEventHandler(() => import("./_chunks/ssr-renderer.mjs"));
var findRoute = /* @__PURE__ */ (() => {
	const data = {
		route: "/**",
		handler: _lazy_IPAi_L
	};
	return ((_m, p) => {
		return {
			data,
