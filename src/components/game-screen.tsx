...
export function GameScreen() {
  ...
  const [kioskTab, setKioskTab] = React.useState<KioskTab>("shop");
  const [hasHandbagNotification, setHasHandbagNotification] = React.useState(false);
  ...
  const [boardTab, setBoardTab] = useState<"highscore" | "xp">("highscore");
  ...
  useEffect(() => {
    fetchOnlineBoard(boardTab === "xp" ? "total" : "single").then((data) => {
      ...
      setBoard(data.map((row) => {
        ...
        return { ...row, totalXp: xp, level: getPlayerLevel(xp) };
      }));
    });
  }, [hud.mode, boardTab, profile.name, profile.totalXp]);
  ...
  return (
    <div className="fixed inset-0 flex h-[100dvh] w-screen items-center justify-center overflow-hidden bg-ink text-paper">
      {!playing && <div className="absolute top-3 right-4 text-[10px] font-mono text-paper-dim/50 font-bold tracking-widest z-50 pointer-events-none">v1.3.4 BETA</div>}
      <div
                  ...
                  <div className="flex flex-col gap-2 rounded-md border border-line bg-ink-3 px-3 py-2">
                    <div className="flex items-center justify-between gap-1.5">
                      <div className="flex flex-col min-w-0 max-w-[100px]">
                        ...
                        <span className="text-xs font-bold text-paper truncate">{getActiveBadgeIcon(profile?.equipped) && <span className="mr-1 text-sm">{getActiveBadgeIcon(profile?.equipped)}</span>}{profile.name}</span>
                      </div>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <button
                  ...
                  className="flex flex-col justify-between rounded-xl border border-emerald-500/30 bg-emerald-950/20 p-2.5 text-left transition-all active:scale-95 hover:bg-emerald-950/30 cursor-pointer shadow-sm"
                >
                ...
                {(() => {
                  ...
                  return (
                    <button
                      ...
                      className="flex flex-col justify-between rounded-xl border border-amber-500/30 bg-amber-950/20 p-2.5 text-left transition-all active:scale-95 hover:bg-amber-950/30 cursor-pointer shadow-sm"
                    >
                      <div className="flex items-center justify-between w-full mb-1">
                        <span className="text-xl">🎁</span>
                        {canClaim ? (
                          <span className="px-1.5 py-0.5 rounded-full bg-red-500 text-white text-[9px] font-black animate-pulse shadow">
                            NEU!
                          </span>
                        ) : (
                          <span className="text-[9px] text-neutral-400 font-mono font-medium">
                            {"Tag " + dailyStatus.streak + "/7 ✓"}
                          </span>
                        )}
                      </div>
                      <div>
                        <div className="text-xs font-bold text-amber-200">Login-Bonus</div>
                        <div className="text-[10px] text-amber-400/70">Täglich Groschen</div>
                      </div>
                    </button>
                  );
                })()}
              ...
              <div className="w-full max-w-[360px] rounded-lg border border-[#5c3a21]/60 p-2 shadow-lg" style={{ backgroundColor: "rgba(10, 8, 6, 0.45)" }}>
                <div className="mb-2 flex items-center justify-between gap-2">
                  ...
                  <div className="flex rounded-md border border-line overflow-hidden">
                    <button type="button" onClick={() => setBoardTab("highscore")} className={`px-2 py-0.5 text-[10px] font-bold ${boardTab === "highscore" ? "bg-amber-500 text-ink" : "bg-ink text-paper-dim"}`}>Rekord</button>
                    <button type="button" onClick={() => setBoardTab("xp")} className={`px-2 py-0.5 text-[10px] font-bold ${boardTab === "xp" ? "bg-amber-500 text-ink" : "bg-ink text-paper-dim"}`}>XP</button>
                  </div>
                  {board.length > 5 && (
                    <button
                      type="button"
      ...
      setInspectedPlayer((prev: any) => prev ? {
        ...
        equipped: acc.equipped || {}
                          ...
                          <span className="flex items-center gap-1.5 truncate">
                            ...
                            <span className="ml-1 text-[10px] font-mono font-bold px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-300 border border-amber-500/40 select-none">Lv.{getPlayerLevel(row.totalXp || row.total_xp || row.score)}</span>
                            {isMe && (
                          ...
                          <span className={`font-mono font-bold ${isMe ? "text-amber-300" : isGold ? "text-amber-400" : isSilver ? "text-slate-200" : isBronze ? "text-orange-400" : "text-amber-400/90"}`}>
                            {boardTab === "xp" ? (row.totalXp || row.score) + " XP" : row.score + " Pkt"}
                          </span>
                        </li>
                      );
                    })}</ol>
        ...
        {(showStatsModal || inspectedPlayer) && (() => {
          const statsTarget: any = inspectedPlayer ? {
          ...
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-ink/80 p-4">
            <div className="flex w-full max-w-sm flex-col items-center rounded-2xl border border-line bg-ink p-6 text-center shadow-2xl">
              ...
              <p className="mt-1 text-xs text-paper-dim">
                Spieler: <span className="font-bold text-amber-400">{getActiveBadgeIcon(statsTarget?.equipped) && <span className="mr-1 text-sm">{getActiveBadgeIcon(statsTarget?.equipped)}</span>}{statsTarget.name}</span>
              </p>
              <div className="my-2 w-full rounded-xl border border-line bg-ink-3 p-4">
              ...
              <button type="button" onClick={() => { setShowStatsModal(false); setInspectedPlayer(null); }} className={primaryBtn}>
                Schließen
              </button>
            </div>
          </div>
        ...
        {showPwModal && (
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-ink/80 p-4">
            <div className="flex w-full max-w-sm flex-col items-center rounded-2xl border border-line bg-ink p-6 text-center shadow-2xl">
              ...
              <p className="mt-1 text-xs text-paper-dim">
                Spieler: <span className="font-bold text-amber-400">{getActiveBadgeIcon(profile?.equipped) && <span className="mr-1 text-sm">{getActiveBadgeIcon(profile?.equipped)}</span>}{profile.name}</span>
              </p>
              {pwChangeSuccess ? (
                <div className="my-2 w-full rounded-lg border border-emerald-600 bg-emerald-950/60 p-3 text-xs font-bold text-emerald-300">
                  ✅ Passwort erfolgreich geändert!
                </div>
              ) : (
                <div className="my-2 flex flex-col gap-2.5 w-full">
                  <input
                    type="password"
                    maxLength={32}
                    value={oldPwInput}
                  ...
                  {board.slice(0, 10).map((row, i) => {
                    ...
                    return (
                      <li key={`${row.name}-${i}-${row.score}`} className={itemStyle}>
                        ...
                        <span className="font-mono font-bold text-amber-400/90">{boardTab === "xp" ? (row.totalXp || row.score) + " XP" : row.score + " Pkt"}</span>
                      </li>
                    );
                  })}</ol>
              </div>
