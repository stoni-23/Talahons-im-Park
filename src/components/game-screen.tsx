...
import { qualifies, submitScore, fetchOnlineBoard, type ScoreEntry } from "@/game/scores";
import { loadProfile, saveProfile, resetCurrentProfile, type PlayerProfile, setActiveUserName } from "@/lib/profile";
...
export function GameScreen() {
  ...
  const [profile, setProfile] = useState<PlayerProfile>({ name: "", highScore: 0, gamesPlayed: 0, totalHits: 0 });
  ...
  useEffect(() => {
    if (hud.mode === "results") {
      ...
      if (activeName) {
        ...
        setName(activeName);
        if (hud.score > 0) {
          submitScore(activeName, hud.score).then((up) => {
  ...
  return (
    <div className="relative flex h-[100dvh] w-full items-center justify-center overflow-hidden bg-ink text-paper">
      {!playing && <div className="absolute top-3 right-4 text-[10px] font-mono text-paper-dim/50 font-bold tracking-widest z-50 pointer-events-none">v1.0.4 BETA</div>}
      <div
            ...
            {!named ? (
              <div className="mt-4 flex w-full max-w-xs flex-col gap-2">
                <form className="flex flex-col gap-2" onSubmit={async (e: FormEvent) => {
                  ...
                  setIsEditing(false);
                  const up = await submitScore(cl, hud.score); setBoard(up);
                }}>
