        ...
        {hud.mode === "title" && (
          <div className="absolute inset-0 flex items-center justify-center bg-ink/80 px-4 py-8">
            <div className="flex max-h-full w-full max-w-lg flex-col items-center gap-4 overflow-y-auto">
              ...
              {/* --- SPÄTI & MISSIONS KASTEN --- */}
      <div className="rounded-2xl border border-neutral-800 bg-neutral-900/90 p-3.5 shadow-xl my-3">
        {profile.name && !isEditing && (
        ...
        {profile.name && !isEditing && (
          <div className="grid grid-cols-2 gap-2">
            ...
            <button
              type="button"
              onClick={() => { setKioskTab("inventory"); setHasHandbagNotification(false); setIsKioskOpen(true); }}
              onTouchEnd={(e) => { e.stopPropagation(); setKioskTab("inventory"); setHasHandbagNotification(false); setIsKioskOpen(true); }}
            ...
            >
              <span>👜</span>
              <span>Handtasche ({getOwnedItemsCount(profile?.inventory)})</span>
              {hasHandbagNotification && (
