...
if (!fs.existsSync(file)) {
  console.error("Fehler: src/game/engine.ts nicht gefunden!");
  process.exit(1);
}
...
const newWalk = `      if (t.act === "opa") {
        t.phaseT += dt;
        t.x += t.vx * dt;
        t.rot = Math.sin(t.phaseT * 6) * 0.045;
        t.y = 1180 + Math.abs(Math.sin(t.phaseT * 6)) * 3.5;
        continue;
      }
...
console.log("Prüfe TypeScript-Syntax...");
try {
  execSync("npx tsc --noEmit", { stdio: "inherit" });
  console.log("\n✅ PERFEKT: Keine Fehler!");
} catch {
  console.error("Fehler! Stelle Backup wieder her...");
