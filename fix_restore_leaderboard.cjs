...
console.log('🚀 1. Setze game-screen.tsx auf sauberen Originalzustand zurück...');
const projectRoot = process.cwd();

try {
  execSync('git checkout src/components/game-screen.tsx', { stdio: 'inherit' });
  console.log('✅ src/components/game-screen.tsx erfolgreich auf sauberen Stand zurückgesetzt.');
} catch (e) {
  console.log('ℹ️ git checkout nicht nötig oder fahre fort...');
}

console.log('\n🚀 2. Erstelle bulletproof scores.ts (Alle Accounts + Highscores vereint)...');
...
export async function fetchOnlineBoard(
  ...
  try {
    ...
    const [accRes, hsRes] = await Promise.all([
      ...
      fetch(
        \`\${SUPABASE_URL}/rest/v1/highscores?select=name,score,total_xp,level,created_at&limit=1000&_t=\${timestamp}\`,
        { headers: headers() }
      ).catch(() => null)
    ]);
...
console.log('✅ src/game/scores.ts aktualisiert (Jeder Account existiert dauerhaft in der Bestenliste!).');
...
if (fs.existsSync(gameScreenPath)) {
  ...
  if (changed) {
    ...
    console.log('✅ src/components/game-screen.tsx Fallbacks sicher korrigiert.');
  }
}
...
try {
  ...
  console.log('\n🎉 Build erfolgreich! Der neue Spieler ist sofort wieder da und alle Sortierungen greifen sauber!');
} catch (e) {
