...
console.log('🚀 Starte Reparatur für Klaus45 & dynamische Platzierung...');
...
async function syncKlausDirectly() {
  const SUPABASE_URL = "https://lforuvtpskrnydlburpt.supabase.co";
  const SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imxmb3J1dnRwc2tybnlkbGJ1cnB0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODc4NjAwMDQsImV4cCI6MjEwMzQzNjAwNH0.dXH7H7VhUPYNcMSGztdJT9L6CYZrnJEdj75xAXo0RPY";
  const hdrs = {
    apikey: SUPABASE_KEY,
  ...
  console.log('🔄 Aktualisiere Klaus45 in Supabase (13107 XP, Level 3)...');
  try {
    await fetch(`${SUPABASE_URL}/rest/v1/accounts?username=ilike.Klaus45`, {
      method: 'PATCH',
      headers: hdrs,
      body: JSON.stringify({
        stats: {
    ...
    await fetch(`${SUPABASE_URL}/rest/v1/highscores?name=ilike.Klaus45`, {
      method: 'PATCH',
      headers: hdrs,
      body: JSON.stringify({
        score: 8828,
    ...
    console.log('✅ Klaus45 wurde in Supabase fest mit 13.107 XP & Level 3 hinterlegt!');
  } catch (err) {
    console.warn('⚠️ Konnte Supabase nicht direkt aktualisieren:', err.message);
  }
}
